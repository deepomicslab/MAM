...
def build_training_matrix(file_name, peptide, peptide_n_mer):
    ...
    filtered_xb_data = filtered_x_data[filtered_x_data.sequence.str.contains('-') == False]
...
def build_test_matrix(file_name):
    ...
    test_target_matrix = test_target.to_numpy()
...
def make_predictions(dir_names, data_sets, mode):
    ...
    for i in range(5):
        model = keras.models.load_model(os.path.join(dir_names[model_name], 'cnn_model_' + str(i) + '.hdf5'))
...
def generate_peptide(dir_names, y_pred, cam_matrix, data_set):
    ...
    with open(os.path.join(dir_names['results_generation'], "generation.txt"), 'w') as f:
        f.write('new_peptide' + '\n')
...
def write_predictions(dir_names, y_predict, mode):
    ...
    # write prediction to new file
    with open(os.path.join(dir_names['results'], "predictions.txt"), 'w') as f:
...
def evaluate(dir_names, mode):
    ...
    write_predictions(dir_names, y_predict, mode)
...
def draw_tsne(cam_matrix, mutate_after_test, dir_names, mode):
    ...
    cams = np.zeros((5, len(x_test), len(x_test[1])))
    for i in range(5):
        model = keras.models.load_model(os.path.join(dir_names['HLA-CNN_models_new'], 'hla_cnn_model_' + str(i) + '.hdf5'))
