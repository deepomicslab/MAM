...
def train(params, dir_names):
    ...
    embed_shape = hla_vec_embed.vectors.shape
    ...
    i = 0
    while True:
        model = keras.models.load_model(os.path.join(dir_names['CNN_models'], 'cnn_model_' + str(i) + '.hdf5'))
        model.compile(loss='binary_crossentropy',
                      optimizer=keras.optimizers.Adam(lr=lr),
                      metrics=['accuracy'])
        model.summary()
        early_stopping = keras.callbacks.EarlyStopping(monitor='loss', patience=2, verbose=1, mode='auto')
