...
def run(params, dirnames):
    ...
    model = Word2Vec(sentences, min_count=min_count, vector_size=dim, window=window, sg=utils.str2bool(params['sg_model']), epochs=int(params['iter']), batch_words=100)
    
    if not os.path.exists(dirnames['Vec_embedding']):
        os.makedirs(dirnames['Vec_embedding'])
    model.save(os.path.join(dirnames['Vec_embedding'], 'Vec_Object'))
